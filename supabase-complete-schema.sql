...
CREATE POLICY "Prevent UPDATE on comments" 
ON comments FOR UPDATE 
USING (false);

-- Prevent deleting comments (only admin can delete via frontend)
CREATE POLICY "Prevent DELETE on comments" 
...
CREATE POLICY "Prevent SELECT on newsletter" 
...
CREATE POLICY "Prevent UPDATE on newsletter" 
ON newsletter FOR UPDATE 
USING (false);

-- Prevent deleting subscriptions (admin manages via frontend)
CREATE POLICY "Prevent DELETE on newsletter" 
...
CREATE POLICY "Allow users to insert read status" 
...
CREATE INDEX IF NOT EXISTS idx_user_notifications_user_email 
ON user_notifications (user_email);
